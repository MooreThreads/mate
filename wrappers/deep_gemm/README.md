...
`mate-deep_gemm` is a compatibility wrapper package that preserves the `deep_gemm` import path on top of MATE GEMM operators on MUSA.
...
Build a wheel from the `wrappers/deep_gemm` directory:
...
```bash
pip install dist/mate_deep_gemm-*.whl
```
