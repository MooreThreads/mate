...
def test_dsa_decode(batch, sq, skv, heads, hkv, dqk, dv, topk, dtype, sm_scale):
    ...
    cache_seqlens = torch.tensor(
        [skv for i in range(batch)], dtype=torch.int32, device=device
    )
    pagesize = 64
    ...
    tl_out, lse = flash_mla_with_kvcache(
        q=q,
...
@pytest.mark.skip(reason="Skip for Metadata Compability")
@tilelang.testing.requires_musa_compute_version_ge(3, 1)
...
def test_dsa_prefill(sq, skv, heads, hkv, dqk, dv, topk, dtype, sm_scale):
    ...
    tl_out, _, _ = flash_mla_sparse_fwd(
        q=q,
        kv=kv,
        indices=indices,
        sm_scale=sm_scale,
