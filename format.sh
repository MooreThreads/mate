...
echo 'MATE codespell: Check Start'
...
echo 'MATE codespell: Done'

echo 'MATE ruff: Check Start'
...
echo 'MATE clang-format: Check Start'
...
if command -v clang-format &>/dev/null; then
    ...
    clang_format_all() {
        find . -type f \( -name '*.c' -o -name '*.cc' -o -name '*.cpp' -o -name '*.h' -o -name '*.hpp' -o -name '*.mu' -o -name '*.muh'\) \
            -not -path "./3rdparty/*" \
            -not -path "./build/*" \
            -exec clang-format -i {} +
    }

    # Format changed C/C++ files relative to master branch
    clang_format_changed() {
        ...
        if ! git diff --diff-filter=ACM --quiet --exit-code "$MERGEBASE" -- '*.c' '*.cc' '*.cpp' '*.h' '*.hpp' '*.mu' '*.muh' &>/dev/null; then
            git diff --name-only --diff-filter=ACM "$MERGEBASE" -- '*.c' '*.cc' '*.cpp' '*.h' '*.hpp' '*.mu' '*.muh' \
